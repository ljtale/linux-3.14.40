...
#ifdef TEST_READ
		printk("jie: worker->arg1 %d, arg2 0x%lx, arg3 %lu, pid %d\n",
		(int)e->arg1, (unsigned long)e->arg2, (size_t)e->arg3, (int)sys_getpid());
		printk(KERN_INFO "jie: worker->files 0x%lx, fs 0x%lx\n",
...
__complete(int q_index, int pos){
	...
	do{
		if(e->status == SC_COMPLETE){
			...
			printk("jie: __complete-> complete pos %d in queue %d, retval %lld\n", pos, q_index, retval);
...
SYSCALL_DEFINE4(asyncos_issue_read, unsigned int, fd, char __user *, buf, size_t, count, int, q_index){
	...
	smp_wmb();
	printk(KERN_INFO "jie: issue->read on fd: %d, buf: 0x%lx, reading %d bytes, returning %d, pid %d\n",
	...
	printk(KERN_INFO "jie: issue->files 0x%lx, fs 0x%lx\n",(unsigned long)current->files, (unsigned long)current->fs);
